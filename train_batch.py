...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    #SUMO
    frame_stack = scene.train_cameras_info.copy()
    frame_indices=list(range(len(frame_stack)))
    ...
    for epoch in range(0, opt.epochs):
        ...
        # 随机选择一个键
        rand_idx_in_keys = randint(0, len(frame_keys) - 1)
        key_to_pop = frame_keys[rand_idx_in_keys] # 获取实际的 Key (例如 'frame_0001')
        
        # 使用真实的键进行 pop 操作
        frames_cams_info = frame_stack.pop(key_to_pop)
